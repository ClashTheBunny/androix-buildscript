...
my $NDK = '/opt/android-ndk-r7';
my $PLATFORM = "14";
my $GCCVERSION = "4.4.3";
my $TARGET = "arm-linux-androideabi";
...
# UPDATE: 2011-12-13.  Changed paths to work with android-ndk-r7 and generalized a bit.
...
my $ALIB = "$NDK/platforms/android-$PLATFORM/arch-arm";
my $PREBUILT  = "$NDK/toolchains/$TARGET-$GCCVERSION/prebuilt/linux-x86";
my $TOOLCHAIN = "$NDK/toolchains/$TARGET-$GCCVERSION";
my $INTERWORK = $PREBUILT;

my $LDSCRIPTS = "$PREBUILT/$TARGET/lib/ldscripts/";

my @include_paths = (
    "-I$NDK/platforms/android-$PLATFORM/arch-arm/usr/include",
  );
...
my @link_args = (
    "-Bdynamic",
    "-Wl,-T,$PREBUILT/$TARGET/lib/ldscripts/armelf_linux_eabi.x",
    ...
    "-lc",
    "-ldl",
    "$PREBUILT/lib/gcc/$TARGET/$GCCVERSION/libgcc.a",
    "-lm"
);
...
my @shared_args = (
    "-nostdlib",
    "-Wl,-T,$PREBUILT/$TARGET/lib/ldscripts/armelf_linux_eabi.xsc",
    "-Wl,--gc-sections",
    "-Wl,-shared,-Bsymbolic",
    "-L$ALIB",
    "-Wl,--no-whole-archive",
