...
mkdir xcb
cd xcb
CC=agcc.pl CFLAGS="-I$INSTALL_PREFIX/include" LDFLAGS="-L/ndk/sysroot/
usr/lib/ -L/ndk/sysroot/usr/lib/static -L$INSTALL_PREFIX/lib" PKG_CONFIG_PATH="/
ndk/androix/usr/lib/pkgconfig:$INSTALL_PREFIX/share/pkgconfig" ../libxcb-1.7/
...
mkdir libX11
cd libX11
CC=agcc.pl CFLAGS="-g -I$INSTALL_PREFIX/include -DISC -DUSE_XMBTOWC -
DX_LOCALE -DWORD64" LDFLAGS="-L/ndk/sysroot/usr/lib/ -L/ndk/sysroot/usr/lib/static
-L$INSTALL_PREFIX/lib" PKG_CONFIG_PATH="$INSTALL_PREFIX/lib/pkgconfig:/ndk/
androix/usr/share/pkgconfig" ../tmzt-androix-lib-libX11-390b7e5/autogen.sh --
host arm-eabi --prefix=$INSTALL_PREFIX
make install
cd ..
mkdir xkbfile
cd xkbfile
CC=agcc.pl CFLAGS="-I$INSTALL_PREFIX/include -fno-short-enums"
